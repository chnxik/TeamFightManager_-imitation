...
namespace ssz
{
	...
	void Champ::InitChampStatus(UINT ATKpt, UINT DEFpt)
	{
		...
		mChampStatus.ChampInfo.ATK = mChampInfo.ATK + ATKpt;    // °ø°Ý·Â
		mChampStatus.ChampInfo.DEF = mChampInfo.DEF + DEFpt;    // ¹æ¾î·Â

		mChampStatus.HP = mChampStatus.ChampInfo.MAXHP;         // ÇöÀç Ã¼·Â
		mChampStatus.CoolTime_Attack = 0.f;						// °ø°Ý ÄðÅ¸ÀÓ
		mChampStatus.CoolTime_Skill = 0.f;						// ½ºÅ³ ÄðÅ¸ÀÓ

		mChampStatus.UltimateUseTime = 60.f;					// ±Ã±Ø±â »ç¿ë ½ÃÁ¡
		mChampStatus.bULTIMATE = false;							// ±Ã±Ø±â »ç¿ë ¿©ºÎ

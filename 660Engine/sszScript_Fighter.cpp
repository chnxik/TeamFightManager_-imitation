...
namespace ssz
{
	...
	void Script_Fighter::Initialize()
	{
		InitChampInfo();
		InitChampAnim();
		InitAudio();
	...
	void Script_Fighter::InitChampInfo()
	{
		...
		Owner->SetName(FIGHTER); // 챔프 이름 입력
	...
	void Script_Fighter::InitChampAnim()
	{
		Champ* Owner = (Champ*)GetOwner();
		...
		std::shared_ptr<Texture> atlas = Resources::Load<Texture>(L"fighter_sprite", L"..\\Resources\\useResource\\ChampSprite\\fighter\\fighter_sprite\\fighter.png");

		// Set MeshRenderer
		LoadMaterial(L"fighter_spriteMt", L"AnimationShader", L"fighter_sprite", eRenderingMode::Transparent);
		Owner->GetComponent<MeshRenderer>()->SetMeshRenderer(L"RectMesh", L"fighter_sprite");

		// SetAnimator
		Animator* anim = Owner->AddComponent<Animator>();

		Vector2 FrmSize(128.f, 128.f);	// Sprite Frm Size


		anim->Create(L"fighter_idle", L"fighter_sprite", Vector2(0.f, 0.f), FrmSize, 5, Vector2(0.f, 0.f), 10.f);
		anim->Create(L"fighter_move", L"fighter_sprite", Vector2(0.f, FrmSize.y * 1), FrmSize, 8, Vector2(0.f, 0.f), 10.f);
		anim->Create(L"fighter_attack", L"fighter_sprite", Vector2(0.f, FrmSize.y * 2), FrmSize, 7, Vector2(0.f, 0.f), 10.f);
		anim->Create(L"fighter_dead", L"fighter_sprite", Vector2(0.f, FrmSize.y * 3), FrmSize, 8, Vector2(0.f, 0.f), 10.f);
		anim->Create(L"fighter_skill", L"fighter_sprite", Vector2(0.f, FrmSize.y * 4), FrmSize, 11, Vector2(0.f, 0.f), 22.f);
		...
		anim->StartEvent(Owner->GetAnimKey(Champ::eActiveType::ATTACK)) = std::bind(&Script_Fighter::Attack, this);
		anim->CompleteEvent(Owner->GetAnimKey(Champ::eActiveType::ATTACK)) = std::bind(&Script_Fighter::AttackComplete, this);
		anim->StartEvent(Owner->GetAnimKey(Champ::eActiveType::SKILL)) = std::bind(&Script_Fighter::Skill, this);
		anim->CompleteEvent(Owner->GetAnimKey(Champ::eActiveType::DEAD)) = std::bind(&BattleManager::RegistRespawnPool, Owner);

		Owner->Play_Idle();
	}

	void Script_Fighter::InitColObj()
	{
		Champ* Owner = (Champ*)GetOwner();
	...
	void Script_Fighter::InitBT()
	{
		...
		Seq_AttackAnim->AddChild<Act_Attack_Fighter>();	// 2-2-2-3-3-1 공격 애니메이션 반복재생

...
namespace ssz
{
	...
	void Script_Ninja::Update()
	{
		Champ_Script::Update();
	}

	void Script_Ninja::InitChampInfo()
	{
		Champ* Owner = (Champ*)GetOwner();

		Owner->SetName(NINJA); // 챔프 이름 입력
		...
		Owner->GetComponent<Transform>()->SetScale(Vector3(256.f, 256.f, 1.f)); // 64 : 128, 96 : 170
	}

	void Script_Ninja::InitChampAnim()
	{
		// ======================================== 공통 애니메이션 세팅 ===========================================
		...
		// Load Atlas
		std::wstring ChampName = L"ninja";							// 클래스이름
		...
		Vector2 DeadAnimInfo = { 10.f, 10.f };
		Vector2 SkillAnimInfo = { 13.f, 10.f };
		...
		anim->StartEvent(Owner->GetAnimKey(Champ::eActiveType::ATTACK)) = std::bind(&Script_Ninja::Attack, this);
		anim->CompleteEvent(Owner->GetAnimKey(Champ::eActiveType::ATTACK)) = std::bind(&Script_Ninja::AttackComplete, this);
		anim->StartEvent(Owner->GetAnimKey(Champ::eActiveType::SKILL)) = std::bind(&Script_Ninja::Skill, this);
		anim->CompleteEvent(Owner->GetAnimKey(Champ::eActiveType::DEAD)) = std::bind(&BattleManager::RegistRespawnPool, Owner);

		Owner->Play_Idle();
	}

	void Script_Ninja::InitColObj()
	{
		Champ* Owner = (Champ*)GetOwner();
	...
	void Script_Ninja::InitBT()
	{
		...
		Seq_AttackAnim->AddChild<Act_Attack_Ninja>();	// 2-2-2-3-3-1 공격 애니메이션 반복재생
	...
	void Script_Ninja::Attack()
	{
		Champ_Script::Attack();
	}

	void Script_Ninja::Skill()
	{
		Champ* Owner = (Champ*)GetOwner();
